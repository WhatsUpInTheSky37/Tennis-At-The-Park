...
export default function Matches() {
  ...
  const statusBadge = (s: string) => {
    ...
    if (s === 'disputed') return <span className="badge badge-red">⚠️ Disputed</span>
    return null
  }
    ...
    <div className="page">
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Recent recorded matches</p>
        </div>
              ...
              <div key={m.id} className="card">
                ...
                {user && m.status === 'pending_confirmation' && [...teams.team1, ...teams.team2].includes(user.id) && !teams.team1.includes(user.id) && (
                  <div className="flex gap-2 mt-3">
                    ...
                    <button className="btn btn-danger btn-sm" onClick={() => setShowDispute(m.id)}>⚠️ Dispute</button>
                  </div>
                )}
                {user && m.status === 'normal' && [...teams.team1, ...teams.team2].includes(user.id) && (
                  <button className="btn btn-ghost btn-sm mt-2" onClick={() => setShowDispute(m.id)}>⚠️ Dispute this match</button>
                )}
              </div>
            )
          })}
